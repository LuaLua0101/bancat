                            ...
                            <ul>
                                ...
                                <li class="d-flex justify-content-between align-items-center bt-line">
                                    ...
                                    <div class="left-col title text-primary">
                                        <img src="{{asset('public/img/post/' . $item->cover)}}" height="80px"
                                            style="margin-right: 10px;">
                                        <a
                                            href="{{route('adgetEditProduct', ['id' => $item->id])}}">{{$item->title}}</a>
                                    </div>
                                    <div class="right-col text-right"><a class="btn delete-btn"
                                            href="{{route('adgetDelProduct', ['id' => $item->id])}}"
                                            onclick="return confirm('Bạn có chắc chắn xóa sản phẩm này?');"><i
